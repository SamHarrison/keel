# keel reference — the complete explanatory documentation

keel is a tailored **ISO/IEC/IEEE 29148:2018** requirements process built
for teams where LLM agents write most of the code: four authored document
layers, one CLI (`tools/req`) that derives every analytical view,
statement-level identity that survives refactors, and three rings of
deterministic gates that run identically in a git hook, an agent's inner
loop, and CI.

**This file explains everything — every file, every schema, every command —
but it is NOT the authority.** `docs/process.md` is the normative core; on
any conflict, the process doc wins. The top-level README.md belongs to the
*project* (keel is the chassis, not the product); the agent contract is
CLAUDE.md. Paths here are repo-root-relative.
  ...
  README.md §How this repo works (the summary)
    → CLAUDE.md            agent contract: must-follow rules + map
      → docs/process.md    THE NORMATIVE CORE (~130 lines)
        ...
        → this file        full explanatory reference (non-normative)
