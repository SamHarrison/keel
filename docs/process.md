...
| `viz` | `--serve`, `--port`, `--project` | Emits `build/viz.html` — the self-contained interactive requirements tracer: expandable flowdown columns (profiles → stakeholders → BRD → scenarios → PRD → criteria), click-to-radiate closure with connector wires, coverage heat modes, a BRD×PRD matrix, search, and a detail panel. Output is derived (`build/` is gitignored). `--serve` runs a localhost server; `--project PATH` points at another keel repo (nothing external to install). |
